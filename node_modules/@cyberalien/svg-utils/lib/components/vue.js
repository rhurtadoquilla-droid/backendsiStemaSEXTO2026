...
function createVueComponent(data, options) {
	...
	if (!fallback && checkForUniqueIDs(stringifiedContent)) stringifiedContent = addReplaceIDsFunctionAsset(imports, assets, options) + "(" + stringifiedContent + ")";
	...
	let content = scriptContent ? `<script setup${useTS ? " lang=\"ts\"" : ""}>
${scriptContent}
<\/script>
${template}
` : template;
