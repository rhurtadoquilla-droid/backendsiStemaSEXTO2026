...
*/
function renderStatefulSVGCSSIconStyle(icon, context, commonStylesheet = createEmptyStylesheet, specialHandlingForFocusState = true) {
	...
	if (context) for (const className in icon.statefulClasses) {
		...
		for (const stateKey in classData.stateRules) {
			...
			if (selectors) for (const tree of selectors) {
				const list = [...tree, baseClassName];
				addGeneratedSelector(stylesheet, specialHandlingForFocusState && isFocusState ? [prefersReduceMotion, ...list] : list, classData.stateRules[stateKey]);
			}
		}
