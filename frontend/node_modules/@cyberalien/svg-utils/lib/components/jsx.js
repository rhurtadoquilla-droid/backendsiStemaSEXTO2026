...
function createJSXComponent(data, options) {
	...
	if (hasFallback) {
		imports.named[fallbackPackage] = /* @__PURE__ */ new Set(["Icon"]);
		dependencies.add(fallbackPackage);
	}
	const reactNamedImports = /* @__PURE__ */ new Set([createElement]);
	...
	return value && value !== defaultValue ? value : undefined;
			...
			if (computedStates.length) {
				componentInternalCode.push(`const states = useMemo(() => ({ ${computedStates.join(", ")} }), [${computedStateNames.map((state) => `${state}Prop`).join(", ")}]);`);
	...
	if (hasComputedViewbox) {
		...
		componentInternalCode.push(`const viewBox = useMemo(() => squareProp ? squareViewBox : baseViewBox, [squareProp]);`);
	} else componentExternalCode.push(`const viewBox = ${getViewBox(viewBox)};`);
	const ratioValue = getViewBoxRatio(viewBox);
	if (hasComputedRatio) componentInternalCode.push(`const ratio = useMemo(() => squareProp ? 1 : ${ratioValue}, [squareProp]);`);
	if (hasFixedSize) {
		...
		componentInternalCode.push(`const size = useMemo(() => ${getSizeProps}(widthProp, heightProp, ${hasComputedRatio ? "ratio" : ratioValue}), [widthProp, heightProp${hasComputedRatio ? ", ratio" : ""}]);`);
		props.width = {
			type: "string",
			value: "width",
			template: "...size,"
	...
	let contentTemplate;
	let stringifiedContent = stringifyFactoryIconContent(data.icon, isEmbeddedCSS ? style : void 0);
	const hasDynamicContent = !hasFallback && checkForUniqueIDs(stringifiedContent);
	if (hasDynamicContent) stringifiedContent = addReplaceIDsFunctionAsset(imports, assets, options) + "(" + stringifiedContent + ")";
	if (!hasFallback) {
		const funcName = addInnerHTMLFunctionAsset(imports, assets, options);
		if (hasDynamicContent) componentInternalCode.push(`const content = useMemo(() => ({__html: ${funcName}(${stringifiedContent})}), []);`);
		else componentExternalCode.push(`const content = {__html: ${funcName}(${stringifiedContent})};`);
...
\t});`);
	...
	const typesCode = useTS ? `interface Props {\n${propTypes}\n};\n\n` : propTypes ? `/** @param {{${propTypes.replace(/\s*\n\s*/g, " ").trim()}}} */\n` : "";
	const usedProps = getUsedFactoryProps(props);
	const componentFunction = `${typesCode}function Component(${usedProps.length ? `{${[...usedProps.map((prop) => `${prop}: ${prop}Prop`), "...props"].join(", ")}}` : "props"}${useTS ? `: Props` : ""}) {
